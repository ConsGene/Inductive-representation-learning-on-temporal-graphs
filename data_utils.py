...
class ScaleUtil:
    ...
    def transform_df(self, original_graph_df, valid_train_flag):
        ...
        if(self.scale_label == 'Discr'):
            columns=[f'label_{i}' for i in range(num_classes)]
            df = pd.DataFrame(np.zeros(shape=(graph_df.shape[0],num_classes)),columns=columns)
        ...
        for cat in self.i2cat.values():
            ...
            graph_df.loc[(graph_df.cat == cat) & valid_train_flag,
                         'label'] = np.clip(orig_labels, lower, upper)
        ...
        if self.scale_label == 'Cbrt':
           ...
           for cat in uniq_cats:
               ...
               transformed_vals = self.scalers_dict[cat].transform(label_vals.reshape(-1, 1))
            ...
            for cat in self.i2cat.values():
                ...
                if (self.scale_label == 'Discr'):
                    self.scalers_dict[cat] = self.scaler(
                        n_bins=self.n_bins, encode=self.encoding)
                else:
                    self.scalers_dict[cat] = self.scaler()
                ...
                graph_df.loc[graph_df.cat == cat, 'label'] = self.scalers_dict[cat].transform(np.array(label_vals_list).reshape(-1, 1))
    ...
    def convert_to_raw_label_scale(self, dst_l_cut, preds):
        if (self.i2cat is None) or (self.scalers_dict is None):
            raise RuntimeError(
                "self.i2cat or self.scalers_dict is None. Run ScaleUtil.transform_df function first")
        raw_preds = []
        for dst, pred in zip(dst_l_cut, preds):
            ...
            if self.scale_label == 'Cbrt':
                ...
                raw_pred = self.scalers_dict[cat].inverse_transform([pred.tolist()])  # Convert numpy array to a list
                if self.scale_label == 'Log':
                    raw_pred = np.sign(raw_pred) * (np.exp(np.abs(raw_pred)) - 1)
                    
                elif self.scale_label == 'Cbrt':
                    raw_pred = np.power(raw_pred, 3)
                    raw_preds.append(raw_pred[0][0])  # Access the value from the list
        if isinstance(preds, np.ndarray):
    ...
    def get_centroid_preds(self,dst_l_cut,preds):
        if (self.i2cat is None) or (self.scalers_dict is None):
            raise RuntimeError("i2cat or scalers_dict is None. Run ScaleUtil.transform_df function first")
        
        centroids = []    
        for dst, pred in zip(dst_l_cut, preds):
            cat = self.i2cat[dst]
            if self.scale_label == 'Discr':
                centroids.append(self.scalers_dict[cat].bin_edges_[pred])
            else:
                centroids.append(pred)
        
        
        return torch.tensor(centroids, dtype=float, device=self.device)
...
class KBinsDiscretizer:
    ...
    def fit(self, X):
        ...
        if self.strategy == "kmeans":
            ...
            kmeans = KMeans(n_clusters=self.n_bins, random_state=0).fit(X.reshape(-1, 1))
