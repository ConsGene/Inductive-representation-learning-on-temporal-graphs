...
parser.add_argument('--scale_label', type=str, choices=['none', 'MinMax', 'Log', 'Cbrt', 'Quantile', 'Discr-quantile', 'Discr-kmeans', 'Discr-quantile-#cat', 'Discr-kmeans-#cat', 'Cluster-kmeans'], default='Discr-kmeans', help='how to scale the label')
...
fh = logging.FileHandler('log/DATA_{}-{}-{}-{}.log'.format(args.data, args.scale_label, args.n_classes, str(time.time())))
...
def eval_one_epoch(tgan, sampler, src, dst, ts, label, label_raw):
    ...
    with torch.no_grad():
        ...
        for k in range(num_test_batch):
            ...
            true_label_raw = np.concatenate([pos_label_raw, np.zeros(size)])
    ...
    return np.mean(val_mae_raw), np.mean(val_r2_raw)
...
for epoch in range(NUM_EPOCH):
    ...
    for k in tqdm(range(num_batch)):
        ...
        with torch.no_grad():
            ...
            neg_label = torch.tensor(get_neg_labels(size), dtype=torch.float, device=device)
        optimizer.zero_grad()
        tgan = tgan.train()
        pos_pred, neg_pred = tgan.contrast(src_l_cut, dst_l_cut, dst_l_fake, ts_l_cut, NUM_NEIGHBORS)
        ...
        with torch.no_grad():
            ...
            true_label = np.concatenate([label_l_cut, get_neg_labels(size)])
    ...
    val_mae_raw, val_r2_raw = eval_one_epoch(tgan, val_rand_sampler, val_data.sources, 
    ...
    if early_stopper.early_stop_check(val_mae_raw):
...
tgan.ngh_finder = full_ngh_finder
test_mae_raw, test_r2_raw = eval_one_epoch(tgan, test_rand_sampler, test_data.sources, 
...
def store_best_metrics(mae_raw, r2_raw):
    ...
    if os.path.exists(BEST_METRICS_PATH):
        ...
        mae_raw_best = float(line_split[1])
    ...
    with open(BEST_METRICS_PATH, 'w') as f:
        ...
        f.write('{}\t{}\t{}\n'.format(MODEL_SAVE_PATH, mae_raw, r2_raw))
        f.close()
    logger.info('Saving model')
    torch.save(tgan.state_dict(), MODEL_SAVE_PATH)
    logger.info('Model saved')

store_best_metrics(test_mae_raw, test_r2_raw)
